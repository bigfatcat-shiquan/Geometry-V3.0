...
$(document).ready(function() {
	...
	$("#draw_canvas").on("click", function(e){
		if (current_draw === "point") {
			...
			addPoint(e.pageX - canvas_left, e.pageY - canvas_top);
		}
	});

	// 添加点及其关联所有事件
	addPoint = function(location_x, location_y, name=null) {
		if (name === null) {
			name = String.fromCharCode(current_point_name_ascii++);
		}
		var click_coordinate = new Point(location_x, location_y);
		...
		new_point_shape.onClick = function() {
			...
			if (current_draw === "delete") {
				...
				this.group.remove();
				this.as_first_of_lines.forEach((line) => line.remove());
				this.as_last_of_lines.forEach((line) => line.remove());
			}
		}
	}
	...
	$("#button_submit_input_condition").on("click", function() {
		...
		if (checkMathStrInput([condition_str_left, condition_str_right])) {
			...
			if (initial_equals_str_set.has(the_new_condition)) {
				...
				addNewCondition(the_new_condition);
			}
		}
	});

	// 已知条件添加
	addNewCondition = function(the_new_condition) {
	...
	$("#button_submit_need_prove").on("click", function() {
		...
		if (checkMathStrInput([condition_str_left, condition_str_right])) {
			...
			if (need_prove_equal_str != null) {
				...
				setNeedProve(the_need_prove);
			}
		}
	});

	// 待求证等量关系指定
	setNeedProve = function(the_need_prove) {
